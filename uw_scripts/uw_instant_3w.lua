...
local _Session = {
    ...
    m_StratTeam = 1,
    m_CompTeam = 5, -- 6 (MPI)
    m_CompTeam2 = 3, -- SAME AS AWARE_V12
    m_TurnCounter = 0,
    m_MyGoal = 0,
    m_AwareV13 = 0,
    m_MyForce = 0,
...
function SetRaceTeamColor()
    ...
    local cpuTeam2 = _Session.m_CompTeam2
    
    -- Set Teamcolor for CPU based on race
    if racePlayer == raceCPU then
        if raceCPU =="f" then
            SetTeamColor(cpuTeam, 95, 180, 120) -- Light Green for Scions #1
            SetTeamColor(cpuTeam2, 135, 100, 180) -- Light Purple for Scions #2
            print("TEAMCOLOR :: SCIONS")
        elseif raceCPU =="i" then
            SetTeamColor(cpuTeam, 90, 145, 200) -- Light Blue for ISDF #1 (75, 140, 220)
            SetTeamColor(cpuTeam2, 100, 160, 120) -- Light Green for ISDF #2 (90, 120, 100)
            print("TEAMCOLOR :: ISDF")
        elseif raceCPU =="j" then
            SetTeamColor(cpuTeam, 90, 145, 200) -- Light Blue for ISDF (Classic) #1 (75, 140, 220)
            SetTeamColor(cpuTeam2, 100, 160, 120) -- Light Green for ISDF (Classic) #2 (90, 120, 100)
            print("TEAMCOLOR :: ISDF")
        else
...
function Start()
    ...
    _Session.m_CompTeam2 = 3; -- AWARE_V12
...
function Update()
    ...
    if (_Session.m_StartDone == false) then
        ...
        if (customCPURecycler ~= nil) then
            ...
            _Session.m_EnemyRecycler2 = BuildStartingVehicle(_Session.m_CompTeam2, _Session.m_CPUTeamRace, customCPURecycler, "*vrecy", "RecyclerEnemy2");
        else
            _Session.m_EnemyRecycler = BuildStartingVehicle(_Session.m_CompTeam, _Session.m_CPUTeamRace, "*vrecycpu", "*vrecy", "RecyclerEnemy");
            _Session.m_EnemyRecycler2 = BuildStartingVehicle(_Session.m_CompTeam2, _Session.m_CPUTeamRace, "*vrecycpu", "*vrecy", "RecyclerEnemy2");
        end
        ...
        -- else
        --     if (_Session.m_CPUTeamRace == string.char(RACE_ISDF)) then
        --         SetAIP("isdfteam.aip", _Session.m_CompTeam2);
        --     else
        --         SetAIP("scionteam.aip", _Session.m_CompTeam2);
        --     end
        end
        ...
        SetScrap(_Session.m_CompTeam2, 40);
...
function SetCPUAIPlan(type)
    ...
    SetAIP(AIPFile .. '.aip', _Session.m_CompTeam2);
